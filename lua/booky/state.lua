...
-- Add a bookmark
function M.add_bookmark(filepath, line_num)
	...
	-- If line_num is provided, check if this specific line is already bookmarked
	if line_num then
		for _, bookmark in ipairs(M.bookmarks) do
			if bookmark.path == normalized and bookmark.line_num == line_num then
				return false -- Line already bookmarked
			end
		end
	else
		-- Check if file is already bookmarked (without line number)
		for _, bookmark in ipairs(M.bookmarks) do
			if bookmark.path == normalized and not bookmark.line_num then
	...
	local name = vim.fn.fnamemodify(normalized, ":t") -- filename only
	if line_num then
		name = name .. ":" .. line_num
	end
	...
	local bookmark = {
		...
		line_num = line_num, -- nil for file bookmarks, number for line bookmarks
	}
